 ...
 * lib/scripts.js
 *
 * Scripts for HTML Mail Helper - Version 1.0
 *
 ...
 *
 * List of available families from data
 */
var families = [];
 ...
 *
 * List of available categories from data
 */
var categories = [];
...
function setLabels() {
  ...
  document.getElementById('phone').placeholder = labels.cu_phone;
...
function init() {
  ...
  // Initialize families data list
  initSelectOptions(families, 'families');

  // Initialize categories data list
  initSelectOptions(categories, 'categories');

  // Initialize civilities data list
 ...
 * @param String selectObjectId, the DOM select object identifier
 * @param String allValuesLabel, the text to select all values (option value is empty)
 */
function initSelectOptions(list, selectObjectId) {

  list.sort();

  document.getElementById(selectObjectId).innerHTML = '';

  list.map((elem) => {

    document.getElementById(selectObjectId).innerHTML += '<option>' + elem + '</option>';
  });
}
...
function showItem(elem) {
  ...
  let item = `<item>
    <header><family>#family</family>#sep<title>#title</title></header>
    <text>#text</text>
  </item>`;
...
function generatePhrase(str) {
  ...
  id += 1;
  let out = str.replaceAll('{agent}', agent.trim())
               ...
               .replaceAll('{phone}', document.getElementById('phone').value.trim())
  ...
  return '<phrase id="phrase' + id + '">'
         + out.replaceAll('\n', '</p><p>')
         + '<up onclick="moveUp(\'phrase' + id + '\')" title="' + labels.phrase_up + '"></up>'
         + '<down onclick="moveDown(\'phrase' + id + '\')" title="' + labels.phrase_down + '"></down>'
         + '<erase onclick="erasePhrase(\'phrase' + id + '\')" title="' + labels.phrase_erase + '"></erase>'
         + '</phrase>';
}
...
function addItem(elem) {
  ...
  while(elem.localName !== 'item') {

      elem = elem.parentElement;
  }

  document.querySelector('RESULT').querySelector('CONTENT').innerHTML += generatePhrase(elem.querySelector('TEXT').innerHTML);
}

/**
 * function filterText()
 *
 * Filter data with text in name
 *
 ...
 */
function filterText(search, text) {
...
function filterData(d) {
  ...
  d.map((elem) => {
    ...
    if(
      ...
      &&
      (searchText.value === '' || (searchText.value !== '' && (filterText(searchText.value, elem.title) || filterText(searchText.value, elem.text))))
    ) {
...
function getPreviousId(id) {
  ...
  try {
    ...
    return document.getElementById(id).previousSibling.id;
 ...
 *
 * Remove phrase from result content
 */
function erasePhrase(id) {
 ...
 * function copyResult()
 *
 * Copy result phrases in clipboard
 */
function copyResult() {
